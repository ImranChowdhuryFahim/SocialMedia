...
class MessageDiv extends Component {
    ...
    componentDidMount() {
        let { top, left } = ReactDOM.findDOMNode(this).getBoundingClientRect()
        ...
        return (
            ...
            <div className='cont'>
                <div className={this.props.cls === 'sent' ? "msgrowsent" : "msgrowrecieved"} >
                    <Tippy content={this.props.msg} placement={this.props.dir}>
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                            <div ref={this.myref} className={this.props.cls === 'sent' ? "msgsent" : "msgrecieved"} data-tooltip={this.props.msg}>
                                <span>{this.props.msg}</span>
                            </div>

                            {this.state.IsEmojiShowing && (
                                <div style={{ boxShadow: '0 2px 4px 1px rgba(0, 0, 0, .1)', zIndex: '999', backgroundColor: 'white', height: '20px', width: 'fit-content', marginTop: '-12px', borderRadius: '10px', display: 'flex', alignContent: 'center' ,marginLeft: (this.props.dir==='right'? this.state.width-29 : this.state.width-29)}}>
                                    <img style={{ height: '16px', width: '16px', padding: '2px', paddingLeft: '4px' }} src={this.state.emoji} alt="nothi"></img>
                                    <span style={{ height: '16px', width: '16px', padding: '2px', paddingLeft: '0px', color: '#0084FF', fontWeight: '540' ,fontSize: '16px'}}>1</span> </div>
                            )
                            }
                        </div>
                    </Tippy>
                    <div>
                        {this.state.IsEmojiPickerShowing && (
                            <ReactionPicker left={this.state.left} top={this.state.top} width={this.state.width} dir={this.props.dir} setMessage={this.handleEmojiSelect.bind(this)} setShowEmojiPicker={this.deactivepicker.bind(this)}></ReactionPicker>)}
                        <div className={(this.state.active === true ? 'active ' : ' ') + (this.props.cls === 'sent' ? "invisibledivsent" : "invisibledivrecieved")}  >
                            ...
                            <span>
                                <Tippy content={'Reply'} >
                                    <button onMouseDown={this.active.bind(this)} onBlur={this.deactive.bind(this)} className="svgbutton" data-tooltip="Reply" >
